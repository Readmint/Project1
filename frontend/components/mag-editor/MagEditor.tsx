...
export function MagEditor({ articleId }: { articleId: string }) {
        ...
        <div className="flex flex-col h-screen bg-slate-50 text-slate-900">
            ...
            <header className="h-16 border-b bg-white flex items-center justify-between px-4 shrink-0 z-50">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="font-bold text-sm">Untitled Design</h1>
                        <p className="text-xs text-slate-500">A4 • {pageCount} Pages</p>
                    </div>
                </div>
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                {/* Left Toolbar */}
                <Toolbar
                    onAdd={(t, c, s) => handleAddElement(t as any, c, s, 1)} // Default toolbar adds to page 1 (since 0 is cover) ??? OR allow active page selection?
                    contentSuggestions={[
                        ...
                        { label: "Article Body", content: articleData.body }, // Can be potentially large
                    ]}
                    attachments={attachments}
                />

                {/* Center Canvas Area - Render Multiple Pages */}
                <div className="flex-1 bg-slate-100 overflow-auto relative flex flex-col items-center p-8 gap-1 cursor-grab active:cursor-grabbing">
                    {Array.from({ length: pageCount }).map((_, pageIndex) => (
                        <div key={pageIndex} className="relative group">
                            {/* Page Label */}
                            <div className="absolute -left-12 top-0 text-xs text-slate-400 font-medium pt-2">
                            ...
                            {pageIndex === 0 ? (
                                ...
                                <Canvas
                                    ...
                                    onDoubleClick={(x: number, y: number) => handleAddElement("text", "Text", { x, y, width: 150, height: 30, fontSize: "16px" }, pageIndex)}
                                    scale={scale}
                                />
                            )}
                        </div>
