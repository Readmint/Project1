...
export default function ChatbotEditor({
  ...
  async function handleCreateTicket() {
    ...
    try {
      ...
      const userName = currentUser.name || currentUser.displayName || userData?.name || "Unknown Editor";
      const userRole = currentUser.role || userData?.role || "editor";
      const userId = currentUser.uid || currentUser.id || `local-${Date.now()}`;

      const ticketData = {
        subject: ticketSubject || "Editor assistance request",
