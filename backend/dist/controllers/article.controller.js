...
const downloadAttachment = async (req, res) => {
    try {
        ...
        if (!res.headersSent)
            res.status(500).json({ status: 'error', message: 'Failed to download attachment', error: err?.message });
        return;
    }
};
...
const deleteAttachment = async (req, res) => {
    try {
        ...
        if (!res.headersSent)
            res.status(500).json({ status: 'error', message: 'Failed to delete attachment', error: err?.message });
        return;
    }
};
    ...
    try {
        ...
        res.status(200).json({
            ...
            data: {
                ...
                attachments: attachments.map((a) => ({ ...a, uploaded_at: a.uploaded_at?.toDate ? a.uploaded_at.toDate() : a.uploaded_at })),
                workflow: workflow.map((w) => ({ ...w, created_at: w.created_at?.toDate ? w.created_at.toDate() : w.created_at })),
                reviews: reviews.map((r) => ({ ...r, created_at: r.created_at?.toDate ? r.created_at.toDate() : r.created_at }))
            }
        });
        return;
    }
